...
// 图片资源
const logoSrc = new URL('@renderer/assets/svg/newLogo.svg', import.meta.url).href
const searchSrc = new URL('@renderer/assets/svg/search.svg', import.meta.url).href

const placeholder = '请输入您想要查找的文档'
...
const removeUserSearch = (item) => {
  ...
  if (i > -1) userSearch.value.splice(i, 1)
...
const removeSearchHistory = async (searchWord) => {
  ...
  if (!userId || !searchWord) return

  const param = { userId, searchWord }
  try {
